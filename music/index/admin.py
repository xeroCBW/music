...
class SongAdmin(admin.ModelAdmin):
    # 设置模型字段，用于Admin后台数据的表头设置
    list_display = ['song_id','song_name','song_singer','song_ablum','song_language','song_release']
    # 设置可搜索的字段并在Admin后台数据生成搜索框，如有外键应使用双下划线连接两个模型的字段
    search_fields = ['song_name','song_singer','song_ablum','song_language']
    # 设置过滤器，在后台数据的右侧生成导航栏，如有外键应使用双下划线连接两个模型的字段
    list_filter = ['song_singer','song_ablum','song_language']
    # 设置排序方式
    ordering = ['song_id']
